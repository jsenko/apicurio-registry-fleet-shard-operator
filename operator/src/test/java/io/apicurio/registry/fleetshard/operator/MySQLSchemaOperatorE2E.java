...
import io.apicurio.registry.fleetshard.operator.crd.v1.ApicurioRegistryFleetShardSpec;
...
import io.apicurio.registry.fleetshard.operator.dependent.SchemaDependentResource;
...
/*
  ...
  AbstractOperatorExtension operator =
      isLocal()
          ? LocallyRunOperatorExtension.builder()
              .withReconciler(new ApicurioRegistryFleetShardReconciler()) // configuration for schema comes from
  ...
  void test() {
    ...
    ApicurioRegistryFleetShard testSchema = new ApicurioRegistryFleetShard();
    testSchema.setMetadata(
        new ObjectMetaBuilder().withName(TEST_RESOURCE_NAME).withNamespace(operator.getNamespace())
            .build());
    testSchema.setSpec(new ApicurioRegistryFleetShardSpec());
    ...
    await()
        ...
        .untilAsserted(
            () -> {
              ApicurioRegistryFleetShard updatedSchema =
                  client
                      .resources(ApicurioRegistryFleetShard.class)
    ...
    client.resources(ApicurioRegistryFleetShard.class).inNamespace(operator.getNamespace())
